...
def begin(self, PARAMS=None):
    self.COUNTER = {'DSA':0, 'survived':0}
    self.DATACOUNTER = {'all':0, 'selected':0}

# declare histograms for Analyzer class
def declareHistograms(self, PARAMS=None):

    self.HistInit('nDSA', ';before;after;Counts', 16, 0., 16., 16, 0., 16.)
    self.HistInit('nPAT', ';before;after;Counts', 16, 0., 16., 16, 0., 16.)
...
def analyze(self, E, PARAMS=None):
    ...
    #### SELECTOR: VTX NS NH FPTE HLT
    Filters = E.getPrimitives('FILTER')
    if not Selections.CUTS['goodVtx'].apply(Filters): return

    DSASelections = {muon.idx:Selections.MuonSelection(muon, cutList='DSAQualityCutList') for muon in DSAmuons}
    selectedDSAmuons = [mu for mu in DSAmuons if DSASelections[mu.idx].allOf('q_nStations', 'q_nMuonHits', 'q_FPTE')]
    ...
    HLTMuonMatches = AT.matchedTrigger(HLTMuons, selectedDSAmuons)
    ...
    newSelectedDSAmuons, selectedPATmuons, newSelectedDimuons = AT.replaceDSAMuons(selectedDSAmuons, PATmuons, selectedDimuons, PATSelections, cutList)

    self.COUNTER['DSA'] += len(selectedDSAmuons)
    self.COUNTER['survived'] += len(newSelectedDSAmuons)

    self.HISTS['nDSA'].Fill(len(selectedDSAmuons), len(newSelectedDSAmuons))
    self.HISTS['nPAT'].Fill(len(selectedDSAmuons), len(   selectedPATmuons))

    #print '{} DSA muons before, {} after -> {} PAT muons'.format(len(selectedDSAmuons), len(newSelectedDSAmuons), len(selectedPATmuons))
...
def end(self, PARAMS=None):
    ...
    print self.NAME, self.COUNTER['DSA'], self.COUNTER['survived']
