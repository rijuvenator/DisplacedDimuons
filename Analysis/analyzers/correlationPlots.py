...
CONFIG = {
    ...
    'vtxChi2' : {'AXES':(1000,      0., 250.   ), 'LAMBDA': lambda dim: dim.normChi2                      , 'PRETTY':'vtx #chi^{2}/dof'      },
    ...
    'd0Sig'   : {'AXES':(1000,      0., 100.   ), 'LAMBDA': lambda dim: min(dim.mu1.d0Sig(), dim.mu2.d0Sig()), 'PRETTY':'|d_{0}|/#sigma_{d_{0}}'},
}

# HCONFIG actually contains the histogram configurations
HCONFIG = {}
...
for q1, q2 in pairs:
    ...
    HCONFIG[key]['TITLE' ] = ';' + TITLE2 + ';' + TITLE1 + ';Counts'
    HCONFIG[key]['AXES'  ] = CONFIG[q2]['AXES'] + CONFIG[q1]['AXES']
    HCONFIG[key]['LAMBDA'] = (CONFIG[q2]['LAMBDA'], CONFIG[q1]['LAMBDA'])

#### CLASS AND FUNCTION DEFINITIONS ####
# declare histograms for Analyzer class
def declareHistograms(self, PARAMS=None):
    for KEY in HCONFIG:
        TITLE = HCONFIG[KEY]['TITLE']
        AXES  = HCONFIG[KEY]['AXES']
        self.HistInit(KEY, TITLE, *AXES)
        if self.SP is not None:
            self.HistInit(KEY+'_Matched', TITLE, *AXES)
...
def analyze(self, E, PARAMS=None):
    ...
    for dimuon in selectedDimuons:
        for KEY in HCONFIG:
            if HCONFIG[KEY]['LAMBDA'] is None: continue
            F1 = HCONFIG[KEY]['LAMBDA'][0]
            F2 = HCONFIG[KEY]['LAMBDA'][1]
            self.HISTS[KEY].Fill(F1(dimuon), F2(dimuon), eventWeight)
            ...
            if len(dimuonMatches) > 0:
                for match in dimuonMatches:
                    ...
                    for KEY in HCONFIG:
                        ...
                        F1 = HCONFIG[KEY]['LAMBDA'][0]
                        F2 = HCONFIG[KEY]['LAMBDA'][1]
                        self.HISTS[KEY+'_Matched'].Fill(F1(dimuon), F2(dimuon), eventWeight)
...
if __name__ == '__main__':
    ...
    analyzer.writeHistograms('roots/CorrelationPlots{}{}_{{}}.root'.format('_Trig' if ARGS.TRIGGER else '', ARGS.CUTS))
