...
def declareHistograms(self, PARAMS=None):
    ...
    for d0min, d0max in D0INTERVALS:
        ...
        for __, __, alpha_category_name in ALPHA_CATEGORIES:
            ...
            self.HistInit(
                "cutFlow" + d0intervals_str + alpha_categories_str,
                ";applied cuts;number of events passing",
                30,
                0,
                30,
            )
            self.HistInit(
                "L1TObjectsPerHLTObject_noSelections"
                + d0intervals_str
                + alpha_categories_str,
            ...
            self.HistInit(
                "L1TObjectsPerHLTObject" + d0intervals_str + alpha_categories_str,
                ";number of L1 objects per HLT muon;Yield",
                40,
                0,
                10,
            ...
            self.HistInit(
                "lowerLegMu_HLTmatches" + d0intervals_str + alpha_categories_str,
                ";HLT matches for lower leg muon;Yield",
                2,
                0,
                2,
            )
            self.HistInit(
                "upperLegMu_HLTmatches" + d0intervals_str + alpha_categories_str,
                ";HLT matches for upper leg muon;Yield",
                2,
                0,
                2,
            )
            self.HistInit(
                "lowerLegMu_HLTmatches_upperLegMu_HLTmatches"
                + d0intervals_str
                + alpha_categories_str,
                ";HLT matches for both lower and upper leg muons;Yield",
                2,
                0,
                2,
            )
            self.HistInit(
                "DSAmuonMultiplicity_noSelections"
                + d0intervals_str
                + alpha_categories_str,
                ";DSA muon multiplicity; Yield",
                29,
                1,
                30,
            )
            self.HistInit(
                "DSAmuonMultiplicity" + d0intervals_str + alpha_categories_str,
                ";DSA muon multiplicity; Yield",
                29,
                1,
                30,
            ...
            self.HistInit(
                "dimuonMultiplicity" + d0intervals_str + alpha_categories_str,
                ";dim. multiplicity;Yield",
                10,
                0,
                10,
            )
                    ...
                    if DO_CREATE_TURNON_HISTS:
                        ...
                        for L1threshold in L1THRESHOLDS:
                            for threshold in PTTHRESHOLDS:
                                self.HistInit(
                                    MUONTYPE
                                    + "_pTGT{}".format(str(threshold).replace(".", "p"))
                                    + "_L1pTGT{}".format(
                                        str(L1threshold).replace(".", "p")
                                    )
                                    + "__"
                                    + KEY
                                    + "VAR"
        ...
        for alpha_min, alpha_max, alpha_category_name in ALPHA_CATEGORIES:
            ...
            for MUONTYPE, MUONS in (("DSA", DSAmuons),):
                ...
                if len(selected_muonpairs) == 0:
                    ...
                    self.HISTS["cutFlow" + d0intervals_str + alpha_categories_str].Fill(
                        10
                    )

                buffer_selected_muonpairs = []  # stores next subset of muon pairs
                buffer_HLTmatches_selected_muonpairs = (
                    []
                )  # stores corresponding HLT matches
                ...
                for m1, m2 in selected_muonpairs:
                    ...
                    if DO_REQUIRE_BOTH_LEGS_MATCHED:
                        m1_passes_first_cut = any(
                            [
                                ...
                                for mu in L1Tmuons
                                if mu.idx
                                == HLTmatches_selected_muonpairs[cnt_pos_muonpairs][0].idx
                            ]
                        )
                        ...
                        m2_passes_first_cut = any(
                            [
                                ...
                                for mu in L1Tmuons
                                if mu.idx
                                == HLTmatches_selected_muonpairs[cnt_pos_muonpairs][1].idx
                            ]
                        )
                    ...
                    if (
                        "noOppositeMuonMatch" in alpha_category_name
                    ...
                    if m1 not in selected_muons:
                        selected_muons.append(m1)
                    if m2 not in selected_muons:
                        selected_muons.append(m2)
                    ...
                    if DO_CREATE_SIMPLE_HISTS:
                        for KEY in CONFIG:
                            ...
                            if KEY in PAIRVARIABLES:
                                ...
                                for dimuon in DIMUONS:
                                    if (
                                        dimuon.idx1 == m1.idx and dimuon.idx2 == m2.idx
                                    ) or (
                                ...
                                KEY in L1RESOLUTIONVARIABLES
                                or KEY in L2RESOLUTIONVARIABLES
                            ):
                                pass  # resolution variables are processed further down

                            else:
                                print (
                                    "[ANALYZER WARNING] Key {} not processed".format(KEY)
                                )
                    ...
                    if DO_CREATE_TURNON_HISTS:
                        ...
                        # match lower-HS muon
                        muonMatches = matchedMuons(
                            lowerHSmuon, HLTmuons, threshold=MATCHING_THRESHOLD_HLT
                        )
                        if len(muonMatches) > 0:
                            is_matched_lowerHSmuon_HLT = True
                            self.HISTS[
                                "lowerLegMu_HLTmatches"
                                + d0intervals_str
                                + alpha_categories_str
                            ].Fill(1)
                            ...
                            for KEY in L2RESOLUTIONVARIABLES:
                                self.HISTS[
                                    ...
                                    CONFIG[KEY]["LAMBDA"](
                                        (lowerHSmuon, lowerHSmuon_HLTmatch)
                                    )
                                )
                            ...
                            lowerHSmuon_L1muons = [
                                L1Tmuon
                                for L1Tmuon in L1Tmuons
                                if L1Tmuon.idx == lowerHSmuon_HLTmatch.idx
                            ]
                        ...
                        # match upper-HS muon
                        muonMatches = matchedMuons(
                            upperHSmuon, HLTmuons, threshold=MATCHING_THRESHOLD_HLT
                        )
                        if len(muonMatches) > 0:
                ...
                self.HISTS[
                    "dimuonMultiplicity" + d0intervals_str + alpha_categories_str
                ].Fill(dimuon_multiplicity)
                if len(selected_muons) > 0:
                    self.HISTS[
                        "DSAmuonMultiplicity" + d0intervals_str + alpha_categories_str
                    ].Fill(len(selected_muons))
...
if __name__ == "__main__":
    ...
    outputname = parse_filename(
        path="roots/",
        # path='/afs/cern.ch/work/s/stempl/private/DDM/analyzer_roots/',
