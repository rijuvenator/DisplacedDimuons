...
# get all histograms
HISTS = {}
f = R.TFile.Open('../analyzers/roots/SignalResEffPlots.root')
...
#       'RSA_nMuon',
    ...
    for sp in RECOSIGNALPOINTS:
        for key in KEYS:
            ...
            fname = 'pdfs/{}_HTo2XTo4Mu_{}.pdf'.format(key, SPStr(sp))
...
def makeOverlayPerSignalPlots():
    ...
    for sp in RECOSIGNALPOINTS:
        for key in KEYS:
            ...
            fname = 'pdfs/SRE_{}_HTo2XTo4Mu_{}.pdf'.format(key, SPStr(sp))
    ...
    for i, sp in enumerate(RECOSIGNALPOINTS):
        if i == 0:
            ...
            if DenKey is not None:
                hDen = f.Get('{}_HTo2XTo4Mu_{}'.format(DenKey, SPStr(sp)))
                hDen.SetDirectory(0)
        else:
            h.Add(f.Get('{}_HTo2XTo4Mu_{}'.format(key, SPStr(sp))))
            if DenKey is not None:
                hDen.Add(f.Get('{}_HTo2XTo4Mu_{}'.format(DenKey, SPStr(sp))))
...
def makeOverlayGlobalPlots(key, DenKey=None):
    ...
    for i, sp in enumerate(RECOSIGNALPOINTS):
        if i == 0:
            h = {}
            h['DSA'] = f.Get('{}_HTo2XTo4Mu_{}'.format('DSA_'+key, SPStr(sp)))
            h['RSA'] = f.Get('{}_HTo2XTo4Mu_{}'.format('RSA_'+key, SPStr(sp)))
            h['Ext'] = f.Get('{}_HTo2XTo4Mu_{}'.format(ExtraKey  , SPStr(sp)))
            ...
            h['DSA'].Add(f.Get('{}_HTo2XTo4Mu_{}'.format('DSA_'+key, SPStr(sp))))
            h['RSA'].Add(f.Get('{}_HTo2XTo4Mu_{}'.format('RSA_'+key, SPStr(sp))))
            h['Ext'].Add(f.Get('{}_HTo2XTo4Mu_{}'.format(ExtraKey  , SPStr(sp))))
    ...
    canvas.firstPlot.SetMinimum(0.)
    canvas.firstPlot.SetMaximum(1.)
    Cleanup(canvas, 'pdfs/SRE_{}.pdf'.format(key))
...
def makeColorPlot(key):
    ...
    Cleanup(canvas, 'pdfs/SRE_{}_HTo2XTo4Mu_Global.pdf'.format(key))
