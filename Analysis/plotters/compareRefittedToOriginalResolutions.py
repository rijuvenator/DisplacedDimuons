...
def computeFitParameters(MUON, fs, sp, quantity, outputTag=None):
    ...
    if quantity not in DATA[fs][sp][MUON]: DATA[fs][sp][MUON][quantity] = {'mean':0, 'sigma':0}
...
for fs in ('2Mu2J',):
    for sp in SIGNALPOINTS:
        for quantity in ('pT', 'd0', 'dz', 'd0Lin', 'dzLin'):
        #for quantity in ('pT',):
...
for quantity in ('pT', 'd0', 'dz', 'd0Lin', 'dzLin'):
    ...
    for fs in ('2Mu2J',):
        for sp in DATA[fs]:
            ...
            #for MUON in ('DSA', 'REF', 'DSADim'):
            #    print '{:5s} {:4d} {:3d} {:4d} {:6s} {:5s} {:8.4f} {:8.4f}'.format(fs, sp[0], sp[1], sp[2], MUON, quantity,
            #        DATA[fs][sp][MUON][quantity]['mean'],
            #        DATA[fs][sp][MUON][quantity]['sigma'],
            #    )
...
PRETTY = {'pT':'p_{T}', 'd0':'d_{0}', 'dz':'d_{z}', 'd0Lin':'lin. d_{0}', 'dzLin':'lin. d_{z}', 'mean':'Mean', 'sigma':'Sigma'}
