			   ...
			   const std::vector<std::string>& ddmHLTPaths)
{
  ...
  */
  ...
  for (unsigned ipath = 0; ipath < ddmHLTPaths.size(); ipath++) {
    if (!triggerEvent.path(ddmHLTPaths[ipath])) continue;
    if (triggerEvent.path(ddmHLTPaths[ipath])->wasAccept()) {
      fired = true;
      unsigned path_prescale = triggerEvent.path(ddmHLTPaths[ipath])->prescale();
      if (debug)
	std::cout << "DDM trigger path " << ddmHLTPaths[ipath]
		  << " fired; path prescale = " << path_prescale << std::endl;
      if (path_prescale != 1) {
	edm::LogWarning("TriggerBranches")
	  << "+++ Warning: the path " << ddmHLTPaths[ipath]
      ...
      if (debug) {
	const pat::TriggerObjectRefVector triggerObjects(triggerEvent.pathObjects(ddmHLTPaths[ipath]));
	for (unsigned i = 0; i < triggerObjects.size(); i++) {
      ...
      // L1 and HLT objects
      const pat::TriggerFilterRefVector triggerFilters(triggerEvent.pathFilters(ddmHLTPaths[ipath]));
	...
	int l1max_prescale = packed_prescales.getPrescaleForName(ddmHLTPaths[ipath]);
	if (l1max_prescale != 1) {
	  edm::LogWarning("TriggerBranches")
	    << "+++ Warning: the path " << ddmHLTPaths[ipath]
