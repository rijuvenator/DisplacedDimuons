			   ...
			   const edm::Handle<reco::BeamSpot> &beamspotHandle)
{
  ...
  static float mass = .105658375;
    ...
    float energy = pow(pow(mu.p(),2.) + pow(mass,2.), 0.5);
		...
		rsamu_p     .push_back(mu.p     () );
		rsamu_chi2  .push_back(mu.chi2  () );
		rsamu_ndof  .push_back(mu.ndof  () );
		...
		rsamu_nMuonHits   .push_back(n_MuHits);
		rsamu_nDTHits     .push_back(n_DTHits);
		rsamu_nCSCHits    .push_back(n_CSCHits);
		rsamu_nDTStations .push_back(n_DTStats);
		rsamu_nCSCStations.push_back(n_CSCStats);
		...
		rsamu_d0_pv.push_back(fabs(d0_pv));
		rsamu_d0_bs.push_back(fabs(d0_bs));
		rsamu_d0_pv_lin.push_back(fabs(d0_pv_lin));
		rsamu_d0_bs_lin.push_back(fabs(d0_bs_lin));
		rsamu_d0sig_pv.push_back(d0sig_pv);
		rsamu_d0sig_bs.push_back(d0sig_bs);
		rsamu_d0sig_pv_lin.push_back(d0sig_pv_lin);
		rsamu_d0sig_bs_lin.push_back(d0sig_bs_lin);

		rsamu_dz_pv.push_back(fabs(dz_pv));
		rsamu_dz_bs.push_back(fabs(dz_bs));
		rsamu_dz_pv_lin.push_back(fabs(dz_pv_lin));
		rsamu_dz_bs_lin.push_back(fabs(dz_bs_lin));
		rsamu_dzsig_pv.push_back(dzsig_pv);
		rsamu_dzsig_bs.push_back(dzsig_bs);
		rsamu_dzsig_pv_lin.push_back(dzsig_pv_lin);
		rsamu_dzsig_bs_lin.push_back(dzsig_bs_lin);

		if (debug) {
		  std::cout << "RSA muon info: charge = " << mu.charge()
			    << " pt = "  << mu.pt()  << " p = "   << mu.p()
